...
require_once 'FluentPDO/FluentPDO.php';
 ...
 if(isset($_POST['fkmatriz'])&&isset($_POST['fkensayo'])&&isset($_POST['fkmetodo'])) {
  ...
  $datos=array(
     ...
     $fluent->insertInto('detalle_matriz', $datos)->execute();
  ...
  $detalleN=getDetalleNorma($pkmatriz,$fluent);
  $normas=getNormas($fluent);
  $precios=ListaPrecio($pkmatriz,$fluent);
 ...
 function getDetalleNorma($pkmatriz,$fluent) {
        try {
           return $fluent
         ...
         ->select("dm.fkdetalle_matriz,dm.fknorma,CONCAT_WS('-',n.codigo,n.gestion,n.acapite) as norma")->where('d.fkmatriz',$pkmatriz)       
         ->fetchAll();
        } catch (Exception $e) {
            die($e->getMessage());
        }
...
function ListaPrecio($pkmatriz,$fluent) {
        try {
           return $fluent
         ->from('detalle_matriz dm')
         ->join('ensayo p on dm.fkensayo=p.pkensayo')
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer" >
        <button type="button" class="btn btn-default" data-dismiss="modal" style="background-color:#F64;color: #FFF; ">Cerrar</button>
                                                                ...
                                                                <td>
                                                                    <a href="#" onclick="Quitarensayo('<?php echo $pkmatriz; ?>','matriz' ,'<?php echo $d->fkensayo; ?>','<?php echo $d->ensayo; ?>')" class="btn btn-outline btn-danger btn-circle"  style="color: darkred"><i class="fa fa-trash"></i></a>
                                                                </td>
                                                            </tr>
                                            <?php endforeach ?>
