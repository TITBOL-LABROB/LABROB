...
class Detalle_Matriz{
    ...
    public function Listar() {
        try {
           $sql = $this->pdo2->prepare("SELECT distinct dm.pkdetalle_proforma,dm.fkmatriz,dm.fkensayo,e.nombre ensayo,u.nombre as medida,me.nombre as metodo,m.nombre as matriz,dm.costo
            FROM detalle_proforma_matriz dm,ensayo e,matriz m,unidad_medida u,metodo me
     ...
     public function ListaPrecio() {
        try {
           return $this->pdo
         ->from('detalle_proforma_matriz dm')
    ...
    {
         try {
          return $this->pdo->insertInto('detalle_proforma_matriz', $datos)->execute();
        } catch (Exception $e) {
            die($e->getMessage());
        }
    }
    ...
    {
     try {
          return $this->pdo->from('detalle_proforma_matriz p')
                     ->select('MAX(p.pkdetalle_proforma) as id')
                     ->fetch();
        } catch (Exception $e) {
            die($e->getMessage());
        }
    ...
    public function Existedetalle_matriz($fkmatriz,$fkparametro)
    {
       try 
        {
          return $this->pdo
         ->from('detalle_proforma_matriz')
         ->select('detalle_proforma_matriz.*')
    ...
    public function Eliminar($datos)
    {
      try 
        {
           $this->pdo->deleteFrom('detalle_proforma_matriz')
